...
def train_scene(gpu, scene, factor):
    cmd = f"OMP_NUM_THREADS=4 CUDA_VISIBLE_DEVICES={gpu} python train.py -s dtu/DTU/scan{scene} -m {output_dir}/scan{scene} -r {factor} --use_decoupled_appearance --lambda_distortion 1000 --ncc_scale 0.5"
    print(cmd)
    if not dry_run:
        os.system(cmd)
